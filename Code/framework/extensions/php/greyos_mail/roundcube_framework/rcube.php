...
class rcube
{
   ...
   /**
    ...
    public static function raise_error($arg = array(), $log = false, $terminate = false)
    {
		...
		if(strpos($arg['message'], 'Could not connect to') !== false)
			$result = array('__show_error'=>'Oops! Could not connect to mail server! <br> Please check your internet connection or try again later.');
		
		else if ($arg['code'] === 403)
			$result = array('__show_error'=>'Sorry Dude, login failed! <br> Please check your email and password.');

		else
			$result = array('__show_error'=>$arg['message']);
