<?php
    /*
        ...
        File name: table.php (Version: 4.6)
    ...
    */
    ...
    {
        // Attributes
        private $__attr_border = false;

        // Matrix
        private $__matrix = null;

        public function Show($matrix, $attributes = null, $events = null)
        {
            if (empty($matrix))
            {
                HELPERS::Error('Table', 1);
            ...
            foreach ($attributes as $key => $value)
            {
                ...
                else if ($key == 'border' && HELPERS::Is_True($value))
                {
                    $html_tag .= 'border="1" ';
                    $this->__attr_border = true;
                }
                else
                {
                    HELPERS::Error('Table', 6);
        ...
        public function Debug($matrix, $attributes = null, $events = null)
        {
            if (!is_bool($matrix))
            {
                HELPERS::Error('Table', 14);
            ...
            if (HELPERS::Is_Valid_Array($attributes))
            {
                foreach ($attributes as $key => $value)
                {
                    if ($key == 'id')
                        ...
                        array_push($attributes_array, $this->__attr_border);
                    else
                    {
                        HELPERS::Error('Table', 10);
            ...
            if ($matrix === true)
            {
                if (!is_array($this->__matrix))
                    $matrix_array = (array)$this->__matrix;
                else
                    $matrix_array = $this->__matrix;

                $merged_array = array_merge($matrix_array, $attributes_array, $events_array);
            }
            else
