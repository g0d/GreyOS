<?php
    /*
        ...
        File name: link.php (Version: 4.6)
    ...
    */
    ...
    {
        // Attributes
        private $__attr_href = null;
        private $__attr_target = null;
        private $__attr_rel = null;
        ...
        public function Show($content, $attributes, $events = null)
        {
            ...
            {
                HELPERS::Error('Link', 2);

                return false;            
            }

            if (!HELPERS::Parameters_Contain($attributes, 'href'))
            {
                HELPERS::Error('Link', 12);
            ...
            {
                HELPERS::Error('Link', 13);

                return false;
            }

            $html_tag = '<a href="' . $attributes['href'] . '" ';
            $this->__attr_href = $attributes['href'];

            foreach ($attributes as $key => $value)
            {
                if (HELPERS::Is_Empty($value))
                {
                    HELPERS::Error('Link', 5);
                ...
                {
                    $html_tag .= 'target="' . $value . '" ';
                    $this->__attr_target = $value;
                }
                else if ($key == 'rel')
                {
                    $html_tag .= 'rel="' . $value . '" ';
                    $this->__attr_rel = $value;
                }
                else
                {
                    if ($key != 'href')
                    {
                        HELPERS::Error('Link', 6);
            ...
            if (HELPERS::Is_Valid_Array($attributes))
            {
                foreach ($attributes as $key => $value)
                {
                    if ($key == 'id')
                        ...
                        array_push($attributes_array, $this->__attr_href);
                    else if ($key == 'target')
                        array_push($attributes_array, $this->__attr_target);
                    else if ($key == 'rel')
                        array_push($attributes_array, $this->__attr_rel);
                    else
                    {
                        HELPERS::Error('Link', 10);
