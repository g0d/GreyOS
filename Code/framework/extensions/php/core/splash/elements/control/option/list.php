<?php
    /*
        ...
        File name: list.php (Version: 4.6)
        Description: This file contains the "CLIST" class.
    ...
    */
    ...
    class CLIST extends OPTION
    {
        ...
        public function Show($options, $attributes, $events = null)
        {
            ...
            {
                HELPERS::Error('List', 2);

                return false;
            }

            if (!HELPERS::Parameters_Contain($attributes, 'size'))
            {
                HELPERS::Error('List', 21);

                return false;
            }

            if (!is_int($attributes['size']) || intval($attributes['size'], 10) < 2)
            {
                HELPERS::Error('List', 22);

                return false;
            }

            $html_tag = '<select size="' . $attributes['size'] . '" ';
            $this->__attr_size = $attributes['size'];

            foreach ($attributes as $key => $value)
            {
                if (HELPERS::Is_Empty($value))
                {
                    HELPERS::Error('List', 5);
