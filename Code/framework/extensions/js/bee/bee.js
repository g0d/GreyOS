/*
    ...
    GreyOS Inc. - Bee (Floating windows for GreyOS)
    
    File name: bee.js (Version: 2.8)
    Description: This file contains the Bee - Floating windows extension.
...
function bee()
{
    ...
    function utilities()
    {
        ...
        this.coords = function(event_object, type)
        {
            ...
            if (vulcan.validation.misc.is_undefined(event_object) || !vulcan.validation.numerics.is_integer(type) || 
                type < 1 || type > 2)
                return false;
        ...
        this.edit_win_title = function()
        {
            ...
            var __bee_id = self.settings.general.id(),
                __ctrl_bar = vulcan.objects.by_id(ui_config.window.control_bar.id),
                __old_title = vulcan.objects.by_id(ui_config.window.control_bar.ids.title),
                __pencil = vulcan.objects.by_id(ui_config.window.control_bar.ids.pencil),
                __win_type_class_title = null,
                __title_width = vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.width) - 100,
                __edit_box = document.createElement('input'),
                __handler = null;

            __handler = function(event) { event.preventDefault(); };
            vulcan.events.detach(__bee_id, vulcan.objects.by_id(ui_config.window.control_bar.id), 'mousedown', __handler);
            ...
            vulcan.events.attach(__bee_id, vulcan.objects.by_id(__edit_box.id), 'keydown', __handler);
        ...
        this.update_win_title = function()
        {
            ...
            var __bee_id = self.settings.general.id(),
                __ctrl_bar = vulcan.objects.by_id(ui_config.window.control_bar.id),
                __title_edit_box = vulcan.objects.by_id(__bee_id + '_title_edit_box'),
            ...
            __handler = function(event) { event.preventDefault(); };
            vulcan.events.attach(__bee_id, vulcan.objects.by_id(ui_config.window.control_bar.id), 'mousedown', __handler);

            __handler = function(event) { self.gui.actions.edit_title(event); };
            vulcan.objects.by_id(ui_config.window.control_bar.ids.pencil).onmousedown = __handler;
        ...
        this.draw_bee = function(bee_object)
        {

            var __swarm_object = vulcan.objects.by_id(swarm.settings.id()),
            ...
            __html = '<div id="' + ui_config.window.control_bar.id + '" class="' + ui_config.window.control_bar.classes.container + '">' + 
                     ...
                     '    </div>' + 
                     '    <div id="' + ui_config.window.control_bar.ids.pencil + '" class="' + ui_config.window.control_bar.classes.pencil + '"></div>' + 
                     '    <div id="' + ui_config.window.control_bar.ids.close + '" class="' + ui_config.window.control_bar.classes.close + '"></div>' + 
                     '    <div id="' + ui_config.window.control_bar.ids.separator + '" class="' + ui_config.window.control_bar.classes.separator + '"></div>' + 
                     '</div>' + 
                     '<div id="' + ui_config.window.menu.id + '" class="' + ui_config.window.menu.class + '">' + 
                     ...
                     '</div>' + 
                     '<div id="' + ui_config.window.content.id + '" class="' + ui_config.window.content.classes.container + '">' + 
            ...
            vulcan.objects.by_id(swarm.settings.id()).appendChild(__dynamic_object);
            ...
            vulcan.events.attach(__bee_id, __swarm_object, 'keydown', __handler);

            __handler = function() { bee_statuses.key_pressed(false); };
            vulcan.events.attach(__bee_id, __swarm_object, 'keyup', __handler);

            __handler = function(event) { __bee_gui.actions.dresize(event); };
            vulcan.events.attach(__bee_id, __swarm_object, 'mousemove', __handler);

            __handler = function(event) { __bee_gui.actions.menu.close(event); };
            vulcan.events.attach(__bee_id, document, 'mousedown', __handler);

            __handler = function(event) { __bee_gui.actions.release(event); };
            vulcan.events.attach(__bee_id, document, 'mouseup', __handler);

            __handler = function(event) { __bee_gui.actions.hover.into(event); };
            vulcan.objects.by_id(__bee_id).onmouseover = __handler;

            __handler = function(event) { __bee_gui.actions.hover.out(event); };
            vulcan.objects.by_id(__bee_id).onmouseout = __handler;

            __handler = function(event) { me.coords(event, 1); };
            vulcan.objects.by_id(__bee_id).onmousemove = __handler;

            __handler = function() { __bee_gui.actions.touch(); };
            vulcan.objects.by_id(__bee_id).onmousedown = __handler;

            __handler = function(event) { me.coords(event, 2); me.manage_drag_status(); };
            vulcan.objects.by_id(ui_config.window.control_bar.id).onmousedown = __handler;
            ...
            if (self.settings.general.type() === 1 || self.settings.actions.can_resize.widget())
            {
                ...
                vulcan.objects.by_id(ui_config.window.status_bar.ids.resize).onmousedown = __handler;

            }

            __handler = function(event) { __bee_gui.actions.menu.open(event); };
            vulcan.objects.by_id(ui_config.window.control_bar.ids.icon).onmouseup = __handler;

            __handler = function(event) { self.settings.actions.can_drag.enabled(false); };
            vulcan.objects.by_id(ui_config.window.control_bar.ids.icon).onmousedown = __handler;

            __handler = function(event) { me.manage_casement(event); };
            vulcan.objects.by_id(ui_config.window.menu.id + '_manage_casement').onmousedown = __handler;

            __handler = function(event) { __bee_gui.actions.close(event); };
            vulcan.objects.by_id(ui_config.window.menu.id + '_close').onmousedown = __handler;

            __handler = function(event) { __bee_gui.actions.edit_title(event); };
            vulcan.objects.by_id(ui_config.window.control_bar.ids.pencil).onmousedown = __handler;

            __handler = function(event) { __bee_gui.actions.close(event); };
            vulcan.objects.by_id(ui_config.window.control_bar.ids.close).onmousedown = __handler;

            __handler = function() { return false; };
            vulcan.objects.by_id(ui_config.window.control_bar.id).onselectstart = __handler;
            vulcan.objects.by_id(ui_config.window.status_bar.id).onselectstart = __handler;

            __handler = function(event) { event.preventDefault(); };
            vulcan.events.attach(__bee_id, vulcan.objects.by_id(ui_config.window.control_bar.id), 'mousedown', __handler);
            vulcan.objects.by_id(ui_config.window.status_bar.id).onmousedown = __handler;

            if (!self.settings.actions.can_select_text())
            {

                __handler = function() { return false; };
                vulcan.objects.by_id(ui_config.window.content.ids.data).onselectstart = __handler;

                __handler = function(event) { event.preventDefault(); };
                vulcan.objects.by_id(ui_config.window.content.ids.data).onmousedown = __handler;

            }

            __handler = function(event) { __bee_gui.actions.hover.into(event); };
            vulcan.objects.by_id(ui_config.casement.id).onmouseover = __handler;

            __handler = function(event) { __bee_gui.actions.hover.out(event); };
            vulcan.objects.by_id(ui_config.casement.id).onmouseout = __handler;

            __handler = function(event) { me.coords(event, 1); };
            vulcan.objects.by_id(ui_config.casement.id).onmousemove = __handler;

            __handler = function() { __bee_gui.actions.touch(); };
            vulcan.objects.by_id(ui_config.casement.id).onmousedown = __handler;

            vulcan.objects.by_id(ui_config.window.control_bar.ids.title).style.width = __bee_gui.size.width() - 100 + 'px';
            vulcan.objects.by_id(ui_config.window.content.ids.data).style.height = __bee_gui.size.height() - 88 + 'px';

            if (self.settings.general.type() === 2 && !self.settings.actions.can_resize.widget())
                vulcan.objects.by_id(ui_config.window.status_bar.ids.message).style.width = __bee_gui.size.width() - 22 + 'px';

            else
                vulcan.objects.by_id(ui_config.window.status_bar.ids.message).style.width = __bee_gui.size.width() - 50 + 'px';

            if (self.settings.general.type() === 1 || self.settings.actions.can_resize.widget())
            {

                vulcan.objects.by_id(ui_config.window.status_bar.ids.resize).style.width = 19 + 'px';
                vulcan.objects.by_id(ui_config.window.status_bar.ids.resize).style.height = 19 + 'px';
            ...
            vulcan.objects.by_id(__bee_id).style.zIndex = __z_index + 2;
            vulcan.objects.by_id(ui_config.casement.id).style.zIndex = __z_index + 1;
        ...
        this.remove_bee = function()
        {
            ...
            var __swarm_div = vulcan.objects.by_id(swarm.settings.id()),
                __bee = vulcan.objects.by_id(self.settings.general.id()),
                __casement = vulcan.objects.by_id(ui_config.casement.id);
    ...
    function events_status_settings_object()
    {
        ...
        function gui()
        {
            ...
            this.casemenent_deployed = false;
                            ...
                            for (var j = 0; j < __inner_loops; j++)
                            {
                                ...
                                if (__contexts[i].events[j].id === event_id)
                                {
                                    ...
                                    if (context_id === 'mouse' || context_id === 'key')
                                    {
                                        ...
                                        var __bee_id = self.settings.general.id(),
                                            __bee_id_dom = vulcan.objects.by_id(__bee_id);

                                        vulcan.events.attach(__bee_id, __bee_id_dom, event_id, __contexts[i].events[j].cmd);
    ...
    function drone()
    {
        ...
        this.use = function(new_func_name, new_func_code)
        {
            ...
            if (vulcan.validation.misc.is_undefined(new_func_name) || vulcan.validation.misc.is_undefined(new_func_code))
                return false;
        ...
        this.run = function(existing_func_name, dynamic_func_args)
        {
            ...
            if (!vulcan.validation.alpha.is_string(existing_func_name) || 
                (!vulcan.validation.misc.is_undefined(dynamic_func_args) && !vulcan.validation.misc.is_array(dynamic_func_args)))
                return false;
            ...
            for (var i = 0; i < __drones_num; i++)
            {
                ...
                if (__drones[i].name === existing_func_name)
                {
                    ...
                    if (vulcan.validation.alpha.is_string(__drones[i].code))
    ...
    function settings()
    {
        ...
        function general()
        {
            ...
            this.type = function(val)
            {
                ...
                if (!vulcan.validation.numerics.is_integer(val) || val < 1 || val > 2)
                    return false;
        ...
        function data()
        {
            ...
            function window()
            {
                ...
                this.content = function(val)
                {
                    ...
                    if (!vulcan.objects.by_id(ui_config.window.content.ids.data))
                        return false;

                    vulcan.objects.by_id(ui_config.window.content.ids.data).innerHTML = val;
                ...
                function labels()
                {
                    ...
                    this.title = function(val)
                    {
                        ...
                        if (!vulcan.objects.by_id(ui_config.window.control_bar.ids.title))
                            return false;

                        vulcan.objects.by_id(ui_config.window.control_bar.ids.title).innerHTML = val;
                    ...
                    this.status_bar = function(val)
                    {
                        ...
                        if (!vulcan.objects.by_id(ui_config.window.status_bar.ids.message))
                            return false;

                        vulcan.objects.by_id(ui_config.window.status_bar.ids.message).childNodes[1].innerHTML = val;
            ...
            function casement()
            {
                ...
                this.content = function(val)
                {
                    ...
                    if (!vulcan.objects.by_id(ui_config.casement.ids.data))
                        return false;

                    vulcan.objects.by_id(ui_config.casement.ids.data).innerHTML = val;
                ...
                function labels()
                {
                    ...
                    this.title = function(val)
                    {
                        ...
                        if (!vulcan.objects.by_id(ui_config.casement.ids.title))
                            return false;

                        vulcan.objects.by_id(ui_config.casement.ids.title).innerHTML = val;
                    ...
                    this.status = function(val)
                    {
                        ...
                        if (!vulcan.objects.by_id(ui_config.casement.ids.status))
                            return false;

                        vulcan.objects.by_id(ui_config.casement.ids.status).innerHTML = val;
    ...
    function gui()
    {
        ...
        function mouse()
        {
            ...
            function actual()
            {
                ...
                this.x = function()
                {
                    ...
                    return utils.mouse_coords.actual.pos_x;
                ...
                this.y = function()
                {
                    ...
                    return utils.mouse_coords.actual.pos_y;
            ...
            function relative()
            {
                ...
                this.x = function()
                {
                    ...
                    return utils.mouse_coords.relative.pos_x;
                ...
                this.y = function()
                {
                    ...
                    return utils.mouse_coords.relative.pos_y;
        ...
        function position()
        {
            ...
            this.z_index = function(val)
            {
                ...
                return validate(1, 'z_index', null, val);
            ...
            function opacity()
            {
                ...
                function opacity_settings()
                {
                    ...
                    this.set = function(val)
                    {
                        ...
                        if (!vulcan.validation.numerics.is_float(val) || val < 0.0 || val > 1.0)
                            return false;
                ...
                this.apply = function()
                {
                    ...
                    fx_lib.opacity.apply(self.settings.general.id(), settings.opacity);
                    fx_lib.opacity.apply(ui_config.casement.id, settings.opacity);
                ...
                this.reset = function()
                {
                    ...
                    fx_lib.opacity.reset(self.settings.general.id());
                    fx_lib.opacity.reset(ui_config.casement.id);
            ...
            function fade()
            {
                ...
                function validate(mode, type, option, index, ssd_array)
                {
                    ...
                    else if (mode === 2)
                    {
                        ...
                        fx_lib.fade[type](self.settings.general.id(), me.fx.fade.settings[type].get.last(1), 
                    ...
                    else if (mode === 4)
                    {
                        ...
                        if (vulcan.validation.misc.is_undefined(option) && vulcan.validation.misc.is_undefined(index))
                        {
                        ...
                        if (!vulcan.validation.numerics.is_integer(option) || option < 1 || option > 3 || 
                            !vulcan.validation.misc.is_undefined(index) && (!vulcan.validation.numerics.is_integer(index) || index < 0))
                            return false;
                    ...
                    else
                    {
                        ...
                        var __fade_settings = utils.fade_settings;

                        if (vulcan.validation.misc.is_undefined(ssd_array[0]) || 
                            vulcan.validation.misc.is_undefined(ssd_array[1]) || 
                            vulcan.validation.misc.is_undefined(ssd_array[2]))
                            return false;

                        if ((!vulcan.validation.numerics.is_float(ssd_array[0]) || ssd_array[0] < 0.0 || ssd_array[0] > 1.0) || 
                            (!vulcan.validation.numerics.is_integer(ssd_array[1]) || ssd_array[1] < 0) || 
                            (!vulcan.validation.numerics.is_integer(ssd_array[2]) || ssd_array[2] < 0))
                            return false;
                ...
                function fade_settings()
                {
                    ...
                    this.batch = function(type, step, speed, delay)
                    {
                        ...
                        if (vulcan.validation.misc.is_undefined(type) || !vulcan.validation.alpha.is_string(type) || 
        ...
        function css()
        {
            ...
            function validate(type, context, sub_context, option, val)
            {
                ...
                function sub_context_css_object(mode)
                {
                    ...
                    if (sub_context === null || vulcan.validation.misc.is_undefined(sub_context))
                        return true;
                    ...
                    else
                    {
                        ...
                        __css_object = vulcan.objects.by_id(ui_config[context][mode][sub_context]);
                ...
                if ((context === 'window' || context === 'casement') && 
                    (!vulcan.validation.misc.is_undefined(sub_context) && sub_context !== null))
                    return false;

                var __css_object = vulcan.objects.by_id(ui_config[context].id);
                ...
                if (vulcan.validation.misc.is_undefined(val))
                {
                    ...
                    if (type === 1)
                    {
                        ...
                        if (vulcan.validation.misc.is_undefined(option))
            ...
            function style()
            {
                ...
                this.get = function(context, sub_context, option)
                {
                    ...
                    if (!vulcan.validation.alpha.is_string(context) || 
                        (!vulcan.validation.misc.is_undefined(sub_context) && !vulcan.validation.alpha.is_string(sub_context)) || 
                        (!vulcan.validation.misc.is_undefined(option) && !vulcan.validation.alpha.is_string(option)))
                        return false;
                ...
                this.set = function(context, sub_context, option, val)
                {
                    ...
                    if (!vulcan.validation.alpha.is_string(context)|| 
                        (sub_context !== null && !vulcan.validation.alpha.is_string(sub_context)) || 
                        (option !== null && !vulcan.validation.alpha.is_string(option)) || 
                        !vulcan.validation.alpha.is_string(val))
                        return false;
        ...
        function actions()
        {
            ...
            function menu()
            {
                ...
                this.open = function(event_object)
                {
                    ...
                    if (event_object.button === 0)
                    {

                        fx_lib.visibility.toggle(ui_config.window.menu.id, 1);
            ...
            function casement()
            {
                ...
                this.deploy = function(event_object)
                {
                    ...
                    function swipe_casement()
                    {
                        ...
                        fx_lib.visibility.toggle(ui_config.casement.id, 1);
                        fx_lib.animation.swipe(ui_config.casement.id, 1, 'right', __casement_width, __speed, __step);
                    ...
                    var __pos_x = me.position.left(),
                        __casement = vulcan.objects.by_id(ui_config.casement.id),
                        __casement_width = vulcan.graphics.pixels_value(__casement.style.width),
                    ...
                    if (self.status.gui.fx.fading.into.finished())
                        ...
                        setTimeout(function() { swipe_casement(); }, utils.animating_events.duration());

                    vulcan.objects.by_id(ui_config.window.menu.id + '_manage_casement').innerHTML = 'Hide casement';
                ...
                this.hide = function(event_object, callback)
                {
                    ...
                    if (vulcan.validation.misc.is_undefined(event_object) || 
                        !vulcan.validation.misc.is_undefined(callback) && !vulcan.validation.misc.is_function(callback))
                        return false;
                    ...
                    var __casement = vulcan.objects.by_id(ui_config.casement.id),
                        __casement_width = vulcan.graphics.pixels_value(__casement.style.width),
                        __step = Math.ceil(__casement_width / 23),
                        __speed = Math.ceil(__step / 3);
                    ...
                    else
                    {
                        ...
                        fx_lib.animation.swipe(ui_config.casement.id, 1, 'left', 
                                               __casement_width, __speed, __step, 
                                               function()
                                               {
                    ...
                    vulcan.objects.by_id(ui_config.window.menu.id + '_manage_casement').innerHTML = 'Deploy casement';
            ...
            this.edit_title = function(event_object)
            {
                ...
                if (vulcan.validation.misc.is_undefined(event_object) || bee_statuses.close())
                    return false;

                if (event_object.button === 0)
                {

                    utils.edit_win_title();
            ...
            this.close = function(event_object)
            {
                ...
                function remove_me(this_object)
                {
                    ...
                    var __bee_id = self.settings.general.id(),
                        __swarm_object = vulcan.objects.by_id(swarm.settings.id());

                    vulcan.events.detach(__bee_id, __swarm_object, 'keydown');
                    vulcan.events.detach(__bee_id, __swarm_object, 'keyup');
                    vulcan.events.detach(__bee_id, __swarm_object, 'mousemove');
                    vulcan.events.detach(__bee_id, document, 'mousedown');
                    vulcan.events.detach(__bee_id, document, 'mouseup');
                    ...
                    return swarm.bees.remove(this_object);

                }

                if (event_object.button === 0 && bee_statuses.opened() && !bee_statuses.close())
                {
                    ...
                    function()
                    {
                        ...
                        if (utils.animating_events.in_progress())
                            setTimeout(function() { remove_me(self); }, utils.animating_events.duration());
                    ...
                    owl.status.set(self.settings.general.id(), 'END');
            ...
            this.touch = function()
            {
                ...
                vulcan.objects.by_id(self.settings.general.id()).style.zIndex = __z_index + 2;
                vulcan.objects.by_id(ui_config.casement.id).style.zIndex = __z_index + 1;
            ...
            this.dresize = function(event_object)
            {
                ...
                if (vulcan.validation.misc.is_undefined(event_object) || bee_statuses.title_on_edit() || bee_statuses.close())
                    return false;

                var __bee_id = self.settings.general.id(),
                    __current_width = vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.width),
                    __current_height = vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.height),
                    __casement_width = vulcan.graphics.pixels_value(vulcan.objects.by_id(ui_config.casement.id).style.width),
                    __dynamic_casement_width = 0,
                    __dynamic_right_pos = 0;

                if (event_object.button === 0 && bee_statuses.drag() && self.settings.actions.can_drag.enabled())
                {
                    ...
                    if (__pos_x <= 0 && __pos_y <= 0)
                    {
                        ...
                        vulcan.objects.by_id(__bee_id).style.left = '0px';
                        vulcan.objects.by_id(__bee_id).style.top = '0px';

                        vulcan.objects.by_id(ui_config.casement.id).style.left = __dynamic_casement_width + 'px';
                        vulcan.objects.by_id(ui_config.casement.id).style.top = '0px';
                    ...
                    else
                    {
                        ...
                        if (__pos_x <= 0)
                        {
                            ...
                            vulcan.objects.by_id(__bee_id).style.left = '0px';
                            vulcan.objects.by_id(__bee_id).style.top = __pos_y + 'px';

                            vulcan.objects.by_id(ui_config.casement.id).style.left = __dynamic_casement_width + 'px';
                            vulcan.objects.by_id(ui_config.casement.id).style.top = __pos_y + 'px';
                        ...
                        if (__pos_y <= 0)
                        {
                            ...
                            vulcan.objects.by_id(__bee_id).style.left = __pos_x + 'px';
                            vulcan.objects.by_id(__bee_id).style.top = '0px';

                            vulcan.objects.by_id(ui_config.casement.id).style.left = __pos_x + __dynamic_casement_width + 'px';
                            vulcan.objects.by_id(ui_config.casement.id).style.top = '0px';
                         ...
                         (swarm.settings.bottom() - swarm.area.mouse.y())))
                    {
                        ...
                        vulcan.objects.by_id(__bee_id).style.left = 
                        swarm.settings.right() - (__current_width + __dynamic_casement_width) + 'px';
                        vulcan.objects.by_id(__bee_id).style.top = 
                        swarm.settings.bottom() - __current_height + 'px';

                        vulcan.objects.by_id(ui_config.casement.id).style.left = 
                        swarm.settings.right() - __dynamic_casement_width + 'px';
                        vulcan.objects.by_id(ui_config.casement.id).style.top = 
                        swarm.settings.bottom() - __current_height + 'px';
                    ...
                    else
                    {
                            ...
                            (swarm.settings.bottom() - swarm.area.mouse.y()))
                        {
                            ...
                            if (__pos_x <= 0)
                            {
                                ...
                                vulcan.objects.by_id(ui_config.casement.id).style.left = __dynamic_casement_width + 'px';
                                vulcan.objects.by_id(ui_config.casement.id).style.top = swarm.settings.bottom() - __current_height + 'px';
                ...
                else if (event_object.button === 0 && bee_statuses.resize() && self.settings.actions.can_resize.enabled() && 
                         !bee_statuses.casement_deployed())
                {

                    var __size_x = 0,
                        __size_y = 0,
                        __resize_x_offset = vulcan.graphics.pixels_value(vulcan.objects.by_id(ui_config.window.status_bar.ids.resize).style.width),
                        __resize_y_offset = vulcan.graphics.pixels_value(vulcan.objects.by_id(ui_config.window.status_bar.ids.resize).style.height),
                                    ...
                                    me.position.top() - me.size.height()))
                    {
                        ...
                        if (__new_width >= me.size.max.width() && __new_height >= me.size.max.height())
                        {
                                ...
                                __new_height - __size_y >= me.size.max.height())
                            {
                                ...
                                vulcan.objects.by_id(__bee_id).style.width = me.size.max.width() + 'px';
                                vulcan.objects.by_id(__bee_id).style.height = me.size.max.height() + 'px';

                                vulcan.objects.by_id(ui_config.window.control_bar.ids.title).style.width = 
                                me.size.max.width() - __resize_title_diff + 'px';

                                vulcan.objects.by_id(ui_config.window.content.ids.data).style.height = 
                                me.size.max.height() - __resize_data_diff + 'px';

                                vulcan.objects.by_id(ui_config.window.status_bar.ids.message).style.width = 
                        ...
                        else
                        {
                            ...
                            if (__new_width >= me.size.max.width())
                            {
                                ...
                                if (__new_width - __size_x >= me.size.max.width())
                                {
                                    ...
                                    vulcan.objects.by_id(__bee_id).style.width = me.size.max.width() + 'px';
                                    vulcan.objects.by_id(__bee_id).style.height = __new_height + 'px';
                            ...
                            if (__new_height >= me.size.max.height())
                            {
                                ...
                                if (__new_height - __size_y >= me.size.max.height())
                                {
                                    ...
                                    vulcan.objects.by_id(__bee_id).style.width = __new_width + 'px';
                                    vulcan.objects.by_id(__bee_id).style.height =  me.size.max.height() + 'px';
                            ...
                            __new_height <= me.size.min.height())
                        {
                            ...
                            if (__size_x >= 0 && __size_y >= 0)
                            {
                                ...
                                vulcan.objects.by_id(__bee_id).style.width = me.size.min.width() + 'px';
                                vulcan.objects.by_id(__bee_id).style.height = me.size.min.height() + 'px';
                            ...
                            __new_width < me.size.max.width() && __new_height < me.size.max.height())
                        {
                         ...
                         vulcan.objects.by_id(ui_config.casement.id).style.width = vulcan.objects.by_id(__bee_id).style.width;
                         vulcan.objects.by_id(ui_config.casement.id).style.height = vulcan.objects.by_id(__bee_id).style.height;

                    }

                    if (!vulcan.validation.misc.is_undefined(vulcan.objects.by_id(__bee_id).childNodes[2]))
                    {

                        if (self.settings.data.window.labels.status_bar().length * 7 < 
                            vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.width))
                            vulcan.objects.by_id(__bee_id).childNodes[2].childNodes[1].childNodes[1].className = '';

                        else
                            vulcan.objects.by_id(__bee_id).childNodes[2].childNodes[1].childNodes[1].className = 'marquee';
            ...
            this.release = function(event_object)
            {
                ...
                if (event_object.button === 0 && !bee_statuses.close() && 
                    swarm.status.active_bee() === __bee_id)
                {
                    ...
                    me.position.left(vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.left));
                    me.position.top(vulcan.graphics.pixels_value(vulcan.objects.by_id(__bee_id).style.top));
        ...
        function gui_status()
        {
            ...
            function size_status()
            {
                ...
                this.width = function()
                {
                    ...
                    return vulcan.graphics.pixels_value(vulcan.objects.by_id(self.settings.general.id()).style.width);
                ...
                this.height = function()
                {
                    ...
                    return vulcan.graphics.pixels_value(vulcan.objects.by_id(self.settings.general.id()).style.height);
    ...
    this.on = function(this_event, cmd)
    {
        ...
        if (vulcan.validation.misc.is_undefined(this_event) || vulcan.validation.misc.is_undefined(cmd))
            return false;
    ...
    this.show = function()
    {
        ...
        if (utils.is_lonely_bee(__bee_id))
            return false;
        ...
        if (!utils.draw_bee(self))
        {

            owl.status.set(__bee_id, 'FAIL');

            return false;

        }

        owl.status.set(__bee_id, 'RUN');
